...
// Thu Jun 21 08:52:11 2018 by ROOT version 6.10/09
// from TTree PreSelection/PreSelection
// found on file: gj16_RA2AnalysisTree.root
//////////////////////////////////////////////////////////

#ifndef TreeMakerTreeClass_h
#define TreeMakerTreeClass_h
...
// Header file from the TreeMaker library
#include "TreeMaker/TreeMaker/interface/TreeMaker.h"

// Types
typedef std::vector<std::pair<std::string,std::string> > VBranchType;
typedef std::vector<TLorentzVector>                      VTLorentzVector;

class TreeMakerTreeClass {
public :
   TTree                          *fChain;   //!pointer to the analyzed TTree or TChain
   Int_t                           fCurrent; //!current Tree number in a TChain
   TTree                          *fFriend;
   std::vector<std::string>        VarTypeNames;
   std::vector<TreeTypes>          VarTypes;
   std::map<std::string,unsigned>  nameCache;
   std::vector<TreeObjectBase*>    variables;
   VBranchType                     branchesCombined, branchesVTLorentzVector, branchesVInt, branchesVDouble;
   vector<int>                     newIndex;
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   TreeMakerTreeClass(TTree *tree=0);
   virtual ~TreeMakerTreeClass();
   virtual Int_t                      Cut(Long64_t entry);
           void                       FillFriendTree_JERC();
   ...
   virtual void                       Loop();
           void                       MakeEmptyFriendTree(std::string name = "friendTree");
           std::pair<std::string,int> MakeScanString(VBranchType branches, std::string friendTreeName = "", bool compare = true, bool test = true);
   virtual Bool_t                     Notify();
           VTLorentzVector            RemakeJets();
           VTLorentzVector            RemakeJetsJECup();
           VTLorentzVector            RemakeJetsJECdown();
           VTLorentzVector            RemakeJetsJERup();
           VTLorentzVector            RemakeJetsJERdown();
   virtual void                       Show(Long64_t entry = -1);
           void                       TestJERCBranches(int scanDepth = 25, bool verbose = false);
};

#endif

#ifdef TreeMakerTreeClass_cxx
TreeMakerTreeClass::TreeMakerTreeClass(TTree *tree) : fChain(0), fFriend(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("gj16_RA2AnalysisTree.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("gj16_RA2AnalysisTree.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("gj16_RA2AnalysisTree.root:/TreeMaker2");
...
TreeMakerTreeClass::~TreeMakerTreeClass()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t TreeMakerTreeClass::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t TreeMakerTreeClass::LoadTree(Long64_t entry)
{
...
void TreeMakerTreeClass::Init(TTree *tree)
{
...
Bool_t TreeMakerTreeClass::Notify()
{
...
void TreeMakerTreeClass::Show(Long64_t entry)
{
...
Int_t TreeMakerTreeClass::Cut(Long64_t entry)
{
...
#endif // #ifdef TreeMakerTreeClass_cxx
